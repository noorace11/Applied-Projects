...
def algo_logic():
    less_than_median = RedBlackTree()
    more_than_median = RedBlackTree()
    the_median = None
    ...
    for _ in range(tests):
        ...
        if command == "a":
            if the_median is None:
                less_than_median.insert(number)
                the_median = number
            else:
                if number < the_median:
                    less_than_median.insert(number)
                else:
                    more_than_median.insert(number)
                if more_than_median.size > less_than_median.size + 1:
                    min_val = more_than_median.minimum().get_key()
                    more_than_median.delete(min_val)
                    less_than_median.insert(min_val)

                elif less_than_median.size > more_than_median.size + 1:
                    max_val = less_than_median.maximum().get_key()
                    less_than_median.delete(max_val)
                    more_than_median.insert(max_val)

            if less_than_median.size > more_than_median.size:
                the_median = less_than_median.maximum().get_key()
            elif less_than_median.size == more_than_median.size:
                the_median = (less_than_median.maximum().get_key() + more_than_median.minimum().get_key()) / 2.0
            else:
                the_median = more_than_median.minimum().get_key()

        elif command == "r":
            if less_than_median.search(number) != less_than_median.TNULL:
                less_than_median.delete(number)
            elif more_than_median.search(number) != more_than_median.TNULL:
                more_than_median.delete(number)
            else:
                print("Wrong!")
                continue
            ...
            if less_than_median.size == 0 and more_than_median.size == 0:
                the_median = None
            elif less_than_median.size == more_than_median.size:
                if less_than_median.size == 0:
                    print("Wrong!")
                    continue
                the_median = (less_than_median.maximum().get_key() + more_than_median.minimum().get_key()) / 2.0
            elif less_than_median.size > more_than_median.size:
        ...
        if the_median is not None:
            #I was having issues with returning decimals so consulted: https://stackoverflow.com/questions/1549801/what-are-the-differences-between-type-and-isinstance
            #to figure out whether I use isinstance when it is float and for is_integer: https://stackoverflow.com/questions/152580/whats-the-canonical-way-to-check-for-type-in-python
            if isinstance(the_median, float) and the_median.is_integer():
                print(int(the_median))
            else:
                print(the_median)
        elif command == "r":
            print("Wrong!")
