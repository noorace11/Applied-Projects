...
/**
 * Update interface to show machine 1
 * @param event
 */
void ViewEdit::OnMachine1(wxCommandEvent& event)
{
    GetPicture()->DisplayDialogMachine(this->GetParent(), 1);
    Refresh();
}

/**
 * Update interface to show machine 2
 * @param event
 */
void ViewEdit::OnMachine2(wxCommandEvent& event)
{
    GetPicture()->DisplayDialogMachine(this->GetParent(), 2);
    Refresh();
}
