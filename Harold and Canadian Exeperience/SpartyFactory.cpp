...
std::shared_ptr<Actor> SpartyFactory::Create(std::wstring imagesDir)
{
    ...
    auto mjolnir = std::make_shared<ImageDrawable>(L"Prop", imagesDir +L"/mjolnir1.png");
    mjolnir->SetCenter(wxPoint(25, 26));
    mjolnir->SetPosition(wxPoint(30, 30));
    larm->AddChild(mjolnir);
