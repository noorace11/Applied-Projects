...
void Picture::Save(const wxString& filename)
{
    ...
    root->AddAttribute(L"machineNumber", wxString::Format(L"%d", mMachine1->GetMachineNumber()));
    root->AddAttribute(L"startingFrame", wxString::Format(L"%d", mMachine1->GetStartTime()));
...
void Picture::Load(const wxString& filename)
{
    ...
    int startingFrame = wxAtoi(root->GetAttribute(L"startingFrame", L"0"));

    mMachine1->SetMachineNumber(machineNumber);
    mMachine1->SetStartTime(startingFrame);
...
/**
 * Function to call to display machine
 * @param parent
 * @param machineNum
 */
void Picture::DisplayDialogMachine(wxWindow* parent, int machineNum)
{
    if(machineNum == 1)
    {
        mMachine1->OpenDialogMachine(parent);
    }
    else if(machineNum == 2)
    {
        mMachine2->OpenDialogMachine(parent);
    }
}
